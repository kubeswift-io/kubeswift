...
fn main() {
    let version = std::env::var("KUBESWIFT_VERSION")
        .unwrap_or_else(|_| format!("{}-dev", env!("CARGO_PKG_VERSION")));
    let git_commit =
        std::env::var("KUBESWIFT_GIT_COMMIT").unwrap_or_else(|_| "unknown".to_string());
    let build_date =
        std::env::var("KUBESWIFT_BUILD_DATE").unwrap_or_else(|_| "unknown".to_string());
