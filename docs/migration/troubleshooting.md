...
| `phase=Validating`, `phaseDetail` "waiting for terminating pods on the target node to release resources" | The target fits the guest only once pods already being deleted there are gone, typically a cancelled or failed migration's destination pod | Normal for up to about 30 s (the launcher's grace period). After 2 minutes the migration fails with "insufficient CPU/memory headroom"; a pod stuck terminating that long usually has a finalizer or an unreachable kubelet: `kubectl get pods -A --field-selector spec.nodeName=<target>`. |
...
| `phase=Failed`, `failureReason=DstNeverReady` (live) | The destination pod did not become Ready within 60 s. The `failureMessage` says why, from the pod's status and its latest Warning events: for example `Warning FailedAttachVolume: AttachVolume.Attach failed for volume … failed to attach to node <target>`, `not scheduled: Unschedulable: …` or `container "launcher" waiting: ImagePullBackOff: …`. The same text is recorded as a `DestinationPodNeverReady` Warning event on the SwiftMigration. | Act on the cause the message names. The destination pod is deleted when the migration fails; its events remain for an hour by default: `kubectl get events -n <ns> --field-selector involvedObject.name=<dst-pod>`. `FailedAttachVolume` on Longhorn: Longhorn does not live-migrate a degraded volume (one that does not have all its replicas healthy), and its attachment ticket reads "waiting for volume to migrate to node <target>". Wait until the volume is healthy (`kubectl -n longhorn-system get volumes.longhorn.io <pv>` shows `ROBUSTNESS` `healthy`), then migrate again. Events are quoted only when the controller's role grants `list` on events, as the chart and kustomize install do from v0.15.0. |
...
| `phase=Resuming` stuck for >5min | The destination launcher pod failed to start (image pull, scheduling failure, volume attach, kubelet error) | `kubectl describe pod <guest>` on the destination, or `kubectl logs <guest>` for swiftletd output. If `spec.timeout` runs out, the `Timeout` failure message ends with why the destination pod is not Ready (`destination pod "<guest>" is not Ready: …`), also recorded as a `DestinationPodNotReady` Warning event. |
