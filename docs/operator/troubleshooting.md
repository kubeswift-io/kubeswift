...
- **OCI install:** Use a chart version whose images exist. The chart (as of the fix) defaults image tags to match the chart version. Reinstall or upgrade:
  ```bash
  helm upgrade kubeswift oci://ghcr.io/kubeswift-io/charts/kubeswift --version 0.14.1 -n kubeswift-system \
    --set controllerManager.image.tag=v0.14.1 \
    --set swiftletd.image.tag=v0.14.1
  ```
