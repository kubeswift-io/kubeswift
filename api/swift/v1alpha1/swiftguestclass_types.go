...
type SwiftGuestClassSpec struct {
	...
	//     PVC to snapshot;
	//   - full-state (includeDisk) snapshots are refused, because the disk
	//     export reads a root PVC these guests do not have;
	//   - migration is refused in EVERY mode. Live migration needs shared
	//     storage, and offline migration only repins the guest and copies
	//     nothing, which would leave its disk behind. The guest stays on the
	//     node that holds its disk; to move it, recreate it.
	//
	// Memory snapshots (Tier B, and Tier C without includeDisk) and
	// rootDisk.size work unchanged. This suits fleets of similar, rebuildable
