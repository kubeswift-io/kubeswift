...
- [x] 1.2 Add `--webhook-enabled` flag (default false) and wrap webhook registration in conditional
- [x] 1.3 Add containerPort 9443 to controller-manager Deployment in `config/manager/deployment.yaml`

## 2. Webhook Service

- [x] 2.1 Add `config/manager/service.yaml` (Service selecting controller-manager, port 9443)
- [x] 2.2 Add service to `config/manager/kustomization.yaml`

## 3. TLS Certificate (cert-manager)

- [x] 3.1 Add `config/webhook/issuer.yaml` (self-signed ClusterIssuer for dev)
- [x] 3.2 Add `config/webhook/certificate.yaml` (cert-manager Certificate for webhook Service DNS name)
- [x] 3.3 Add certificate volume and volumeMount to controller-manager Deployment (via overlay patch)
- [x] 3.4 Add issuer and certificate to `config/webhook/kustomization.yaml`
...
- [x] 5.2 Create `config/overlays/webhook/deployment-patch.yaml` (--webhook-enabled=true, cert volume, volumeMount)
- [x] 5.3 Ensure minimal `config/default` remains deployable without webhook
