...
The repository SHALL provide a ValidatingWebhookConfiguration for SwiftGuest, SwiftImage, and SwiftSeedProfile. The webhook SHALL reference the webhook Service. The configuration SHALL include the correct `caBundle` (injected by cert-manager or provided manually).
...
The repository SHALL provide TLS certificate handling for webhook serving. The controller-manager SHALL serve webhooks over TLS. Certificates SHALL be provisioned via cert-manager (Certificate resource) or documented manual process. The controller-manager Deployment SHALL mount the certificate Secret at the cert-dir expected by controller-runtime.
...
- **WHEN** cert-manager CA injector runs (or manual caBundle is set)
...
If cert-manager is used, the repository SHALL document the cert-manager prerequisite clearly. Documentation SHALL include: install command or link, required version, and any namespace or configuration requirements.
